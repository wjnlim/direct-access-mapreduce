...
void worker_context_init(const char* worker_name, const char* worker_ip, 
    ...
    const char* direct_access_mr_home_env = getenv("DIRECT_ACCESS_MR_HOME");
    if (direct_access_mr_home_env == NULL) {
        err_exit("[worker.c: worker_context_init()] DIRECT_ACCESS_MR_HOME env variable not found.");
    }
    sprintf(g_wctx.mr_exe_dir, "%s/%s", direct_access_mr_home_env, MR_EXEC_DIR);
    sprintf(g_wctx.shared_file_list, "%s/%s", direct_access_mr_home_env, SHARED_FILE_LIST);
    sprintf(g_wctx.shared_dir, "%s/%s", direct_access_mr_home_env, SHARED_DIR);
    sprintf(g_wctx.worker_output_dir, "%s/%s", direct_access_mr_home_env, MR_OUTPUT_DIR);
